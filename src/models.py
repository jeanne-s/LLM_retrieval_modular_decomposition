...
def get_model(model_name: str):
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",
        use_auth_token='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'
    )
...
def get_gemma_model(size: str):
    assert size in ["2b", "7b"]
    model_name = f"google/gemma-{size}"
    return get_model(model_name)
...
def get_model_from_name(model_name: str):
    ...
    if "gemma" in model_name:
        return get_gemma_model(model_name.replace("gemma-", ""))
    else:
